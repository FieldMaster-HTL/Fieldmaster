...
describe('FARM_QUERIES', () => {
  ...
  describe('mapClerkIdToLocalId', () => {
    it('should return the local ID if a farm is found', async () => {
      ...
      (db.then as jest.Mock).mockImplementation((callback) => callback([{ id: 'local_farm_id_123' }]));
...
describe('FARM_MUTATIONS', () => {
  ...
  describe('createFarm', () => {
    it('should create a new farm with a creator', async () => {
      ...
      await FARM_MUTATIONS.createFarm('clerk_farm_id_abc', 'Farm Name', 'farm-slug', 'clerk_creator_id_xyz');
    ...
    it('should create a new farm without a creator if created_by lookup fails', async () => {
      (USER_QUERIES.mapClerkIdtoLocalId as jest.Mock).mockRejectedValue(new Error('Creator not found'));
      (db.insert as jest.Mock).mockReturnThis();
      (db.values as jest.Mock).mockResolvedValue([{ id: 'new_farm_id' }]);
  ...
  describe('updateFarm', () => {
    it('should update a farm', async () => {
      ...
      await FARM_MUTATIONS.updateFarm('local_farm_id_123' as any, { name: 'Updated Name', slug: 'updated-slug' });
  ...
  describe('updateFarmByClerkId', () => {
    it('should update a farm by clerk ID', async () => {
      ...
      await FARM_MUTATIONS.updateFarmByClerkId('clerk_farm_id_123', { name: 'Updated Name', slug: 'updated-slug' });

      expect(mapClerkIdToLocalIdSpy).toHaveBeenCalledWith('clerk_farm_id_123');
      expect(updateFarmSpy).toHaveBeenCalledWith('local_farm_id_123', { name: 'Updated Name', slug: 'updated-slug' });
    });
  });
